<template>
  <div>
    ...
    <div v-else class="space-y-4">
      <RequestCard 
        v-for="request in getRequestsByStatus('Refunded')" 
        :key="request._id" 
        :request="request"
        :actionButtons="[
          {
            ...
            class: 'bg-purple-500 hover:bg-purple-600 text-white font-bold py-2 px-4 rounded-md text-sm transition duration-150',
            click: () => updateRequestStatus(request._id, 'Completed')
          }
        ]"
      />
    </div>
